...
const EmployeeList = () => {
  const [employees, setEmployees] = useState([{"id":"01","first_name":"Jaymee","last_name":"Imm","email":"jimm0@craigslist.org","gender":"Genderqueer","age":80},
  {"id":"02","first_name":"Jody","last_name":"Themann","email":"jthemann1@skyrock.com","gender":"Agender","age":29},
  {"id":"03","first_name":"Cece","last_name":"Carlet","email":"ccarlet2@jalbum.net","gender":"Male","age":69},
  {"id":"04","first_name":"Elton","last_name":"Allinson","email":"eallinson3@jugem.jp","gender":"Male","age":31},
  {"id":"05","first_name":"Garvy","last_name":"Shaddick","email":"gshaddick4@rediff.com","gender":"Male","age":32},
  {"id":"06","first_name":"Fin","last_name":"Realy","email":"frealy5@unc.edu","gender":"Male","age":26},
  {"id":"07","first_name":"Minnaminnie","last_name":"Fransseni","email":"mfransseni6@aboutads.info","gender":"Agender","age":52},
  {"id":"08","first_name":"Fernando","last_name":"Pagon","email":"fpagon7@blogs.com","gender":"Male","age":40},
  {"id":"09","first_name":"Tiphanie","last_name":"Fenny","email":"tfenny8@sakura.ne.jp","gender":"Female","age":52},
  ...
  const handleAdd = (employee) => {
    ...
    setEmployees(employees.filter((e) => e.id !== employee.id)); // remove obj that same show in employees and team list
  };

  const handleRemove = (employee) => {
    setTeam(team.filter((e) => e.id !== employee.id)); // remove obj that same show in employees and team list
    setEmployees([employee,...employees]);
  };

  const handleSort = (team) => {
    setTeam([...team.sort((a, b) => a.age - b.age)]);
  };
  
  return (
    <>
      <h1 style={{textAlign : "center"}}>Employee Project</h1>
      <div className='main'>
          ...
          <ol>
            {
              employees.map((employee) => (
                <li className='employee_li' key={employee.id}>
                  {employee.id}.&nbsp;&nbsp;  {employee.first_name}&nbsp;&nbsp; {employee.age}

                  <button onClick={() => handleAdd(employee)}>ADD</button>
                </li>
              ))
            }
          </ol>
        ...
        <div className='team'>
          ...
          <hr/>
          <button className='sort' onClick={() => handleSort(team)}>SORT BY AGE</button>
          <ol>
            {
              team.map((employee) => (
                <li className='employee_li' key={employee.id}>
          ...
          <p className='avg'>Average age: {team.reduce((a, b) => a + b.age, 0) / team.length}</p>
        </div>
      </div>
    </>
  );
